    ...
    function redraw() {
        ...
        if (mouse_mode != RED.state.JOINING) {
            ...
            if (activeSubflow) {
                ...
                outGroup.append("rect").attr("class","port").attr("rx",3).attr("ry",3).attr("width",20).attr("height",10).attr("x",-5).attr("y",15)
                    ...
                    if (d.dirty) {
                        ...
                        if (mouse_mode != RED.state.MOVING_ACTIVE) {
                            ...
                            output_group.append("rect").attr("class","port").attr("rx",3).attr("ry",3).attr("width",10).attr("height",10)
                                ...
                                .on("mouseout",function(d,i) { var port = d3.select(this); port.classed("port_hovered",false);});
                            if (pathCode) {
                            output_group.append("text").attr("class","output_size").attr("x",15).attr("y",10).attr("fill","red").text(function(d) {
                                return pathCode[d].code;
                            });
                            }
            ...
            linkEnter.each(function(d,i) {
                ...
                //通过在path线条上增加箭头能比较直观展现流程走向
                l.append("svg:path").attr("marker-start","url(#arrow)").attr("class","link_line link_path")
